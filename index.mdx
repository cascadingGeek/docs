...
0xmeta is an **x402-compliant payment facilitator** that enables gasless payments on Base networks using EIP-3009 with **post-settlement fee collection**.

<Note>
**Post-Settlement Fees:** Customer payments go directly to your address. The facilitator collects a $0.01 fee from your pre-approved USDC balance after each settlement.
</Note>
...
The facilitator collects fees from your USDC balance via `transferFrom`. This requires one-time approval:
...
sequenceDiagram
    ...
    rect rgb(255, 245, 230)
        Note over Facilitator: Fee Collection
        Facilitator->>Merchant: transferFrom(merchant, treasury, $0.01)
    end
    
    rect rgb(230, 245, 255)
        Note over Facilitator,Merchant: Settlement
        Facilitator->>Merchant: Client → Merchant ($0.02)
    end
...
**Key Points:**
- ✅ Client authorizes payment to **YOUR address** ($0.02)
- ✅ You receive 100% of customer payment
- ✅ Facilitator collects $0.01 fee from your approved balance
...
✅ Facilitator collects $0.01 from YOUR approved balance
