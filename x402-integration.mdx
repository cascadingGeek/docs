...
// ✅ Get treasury address from facilitator's /supported endpoint
...
# Your merchant address (where you'll receive funds after fees)
...
// ✅ Treasury address from /supported endpoint
...
<AccordionGroup>
  <Accordion title="Technical Requirement">
    ...
    **Solution:** Client authorizes to treasury → Treasury forwards to merchant.
  </Accordion>
...
# Production config
EVM_ADDRESS=0x5d791e3554d0e83f171126905bda1640bf6f9a8b
MERCHANT_ADDRESS=0xYourActualMerchantAddress
FACILITATOR_URL=https://facilitator.0xmeta.ai/v1
NODE_ENV=production
PORT=4021
```
    ...
    paymentMiddleware(
      {
        "GET /resource": {
          accepts: [{
            payTo: "0x5d79...",  // ✅ Treasury, not merchant
          }]
        }
      }
    )
